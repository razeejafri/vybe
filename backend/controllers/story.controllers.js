...
export const uploadStory = async (req, res) => {
    try {
        ...
        if(user.story) {                                            //delete already present story
...
export const getStoryByUserName = async (req, res) => {
    try {
        ...
        return res.status(500).json({message: `story get error ${error}`})
    }
}

export const getAlllStories = async (req, res) => {
    try {
        const currentUser = await User.findById(req.userId)
        const followingIds = currentUser.following

        const stories = await Story.find({
            author: {$in : followingIds}                        // author id in the followingids
        ...
        return res.status(500).json({message: `allstories get error ${error}`})
    }
}