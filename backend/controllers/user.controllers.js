...
export const getCurrentUser = async (req, res) => {
    try {
        ...
        populate("saved.author", "name userName profileImage")
...
export const suggestedUsers =async (req, res) => {
    try {
        ...
        return res.status(400).json({message: `get suggested users error ${error}`})
    }
}
...
export const follow =async (req, res) => {
    try {
        ...
        if(currentUserId == targetUserId) {
            return res.status(400).json({message: "not follow"})
        }
...
export const getAllNotifications = async (req, res) => {
    try {
        ...
        return res.status(500).json({message: `notifications error ${error}`})
    }
}
    ...
    try {
        ...
        if(Array.isArray(notificationId)) {
            ...
            await Notification.updateMany(
                { _id: { $in: notificationId }, receiver: req.userId},          //this  id, receiver
                { $set: { isRead: true }}
            );
        } else {
            // mark single notification as read
            await Notification.findOneAndUpdate(
                { _id: notificationId, receiver: req.user },
                { $set: { isRead : true }}
            );
        }
