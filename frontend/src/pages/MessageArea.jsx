...
function MessageArea() {
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            dispatch(setMessages([...messages, result.data]))                                 //purane aise ke aise hi sath me nya wala jod diya
            setInput("")
  ...
  return (
    <div className='w-full h-[100vh] bg-black relative'>
        ...
        px-[40px] flex flex-col gap-[50px] overflow-auto'>
        ...
        items-center bg-black z-[100]'>
            ...
            onSubmit={handleSendMessage}>
                {/* image while sending */}
                {frontendImage && 
                <div className='w-[100px] rounded-2xl h-[100px]
                ...
                h-full px-[20px] text-[18px] text-white outline-0' onChange={(e)=>setInput(e.target.value)}
                value={input}/>
                <div onClick={()=>imageInput.current.click()}><LuImage className='w-[28px] h-[28px] text-white'/></div>
                {(input || frontendImage) &&
                    <button className='w-[60px] h-[40px] rounded-full 
                    bg-gradient-to-br from-[#9500ff] to-[#ff0095] flex 
                    items-center justify-center cursor-pointer' type='submit'><IoMdSend className='w-[25px] 
