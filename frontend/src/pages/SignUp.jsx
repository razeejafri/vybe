...
const SignUp = () => {
  ...
  const handleSignUp = async () => {
    try {
      ...
      dispatch(setUserData(result.data))                                         // slice
        ...
        <div className='w-full lg:w-[50%] h-full bg-white flex flex-col items-center p-[10px] gap-[20px]'>
          ...
          onClick={() => setInputClicked({...inputClicked, name: true})}>
            ...
            <label htmlFor="name" className={`text-gray-700 absolute 
              ...
              <input type="text" id='name' className='w-[100%] h-[100%] rounded-2xl 
              px-[20px] outline-none border-0' value={name} onChange={(e) => setName(e.target.value)} required/>
          </div>
