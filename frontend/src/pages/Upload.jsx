...
function Upload() {
    ...
    const [frontendMedia, setFrontendMedia] = useState(null)                                //works in frontend by url
    const [backendMedia, setBackendMedia] = useState(null)                                  //compelete file(goes in backend)
    const [mediaType, setMediaType] =useState("")                                          //get it file obj
  ...
  return (
    ...
    items-center'>
        ...
        flex justify-around items-center gap-[10px]'>
            ...
            w-[28%] h-[80%] flex justify-center items-center 
        ...
        mt-[15vh] rounded-2xl cursor-pointer hover:bg-[#353a3d]' onClick={()=>mediaInput.current.click()}>             
            {/* inp(hidden) => useRef(to allow clicking on the whole div) */}
            <input type="file" accept={uploadType=="loop"?
                "video/*":""} hidden ref={mediaInput} onChange={handleMedia}/>
        ...
        flex flex-col items-center justify-center mt-[15vh]'>
            ...
            flex flex-col items-center justify-center mt-[5vh]'>
                ...
                {uploadType != "story" && <input type="text" className='w-full 
                border-b-gray-400 border-b-2 outline-none px-[10px] py-[5px] 
                text-white mt-[20px]' placeholder='write caption' onChange={(e)=>setCaption(e.target.value)}
                value={caption}/>}
            </div> }

            { mediaType == "video" && <div className='w-[80%] max-w-[500px] h-[250px]
            flex flex-col items-center justify-center mt-[5vh]'>
        ...
        {frontendMedia && <button className='px-[100px] w-[60%] max-w-[400px]
        py-[5px] h-[50px] bg-[white] mt-[50px] cursor-pointer
        rounded-2xl' onClick={handleUpload}>{loading ? <ClipLoader size={30} color='black'/> : `Upload ${uploadType}`}</button>}
