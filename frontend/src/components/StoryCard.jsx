...
function StoryCard({storyData}) {
    ...
    useEffect(() => {
      const interval = setInterval(() => {
        setProgress(prev=> {
          ...
          return prev+1})
      },150)                                               //function calling itself in every 150 milisecons
  ...
  return (
    ...
    pt-[10px] relative flex flex-col justify-center overflow-hidden'>
        ...
        {!showViewers && 
          <>
            <div className='w-[full]  h-[90vh] flex items-center justify-center'>
                {storyData?.mediaType == "image" && <div className='w-[90%]
                  flex items-center justify-center '>
                    <img src={storyData.media} alt="" className='w-[80%] rounded-2xl 
                     object-cover'/>
                
                  </div> }

              {storyData?.mediaType == "video" && <div className='w-[80%] 
      ...
      {storyData?.author?.userName == userData?.userName &&
          <div className='w-full h-[70px] flex items-center gap-[10px] cursor-pointer text-white p-2 absolute left-0
           bottom-0' onClick={()=>setShowViewers(true)}>
              <div className='text-white flex items-center gap-[5px]'
              > <FaEye /> {storyData.viewers?.length}</div>

              <div className='flex relative'>
              
                  {storyData?.viewers?.slice(0,3).map((viewer, index) => (
                    <div className={`w-[30px] h-[30px] border-2 border-black
                    rounded-full cursor-pointer overflow-hidden ${index>0 ? `absolute left-[${index*10}px]` : `` }`}>
      ...
      {showViewers &&
        <>
          <div className='w-full  h-[30vh] flex items-center justify-center mt-[100px] cursor-pointer py-[30px] overflow-hidden' onClick={() => setShowViewers(false)}>
                {storyData?.mediaType == "image" && <div className='h-full
                  flex items-center justify-center '>
                    <img src={storyData.media} alt="" className='h-[100%] rounded-2xl 
                     object-cover'/>
                
                  </div> }

              {storyData?.mediaType == "video" && <div className='h-full 
              flex flex-col items-center justify-center'>
                <VideoPlayer media={storyData.media}/>
          ...
          <div className='w-full h-[70%] border-t-2 border-t-gray-800 
          p-[20px]'> 
            <div className='text-white flex items-center gap-[10px]'> 
              <FaEye /> <span>{storyData?.viewers?.length}</span> 
              <span>Viewers</span>
            </div>
                {/* viewers */}
