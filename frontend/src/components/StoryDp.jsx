...
function StoryDp({ProfileImage, userName, story}) {
  ...
  return (
    <div className='flex flex-col w-[80px]'>
      ...
      "bg-gradient-to-r from-blue-500 to to-blue-950" :
      ...
      rounded-full flex justify-center items-center relative`} onClick={handleClick}>
        ...
        rounded-full cursor-pointer overflow-hidden'>
          ...
          <div>
            <FiPlusCircle className='text-black absolute bottom-[8px] bg-white 
            right-[10px] rounded-full w-[22px] h-[22px]'/>
          </div>}

        </div>
      </div>
      {/* truncate{... for big usernames} */}
      <div className='text-[14px] text-center truncate w-full
      text-white'>{userName}</div>
    </div>
  )
}
